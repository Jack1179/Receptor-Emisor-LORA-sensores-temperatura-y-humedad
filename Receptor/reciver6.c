...
// Configuración SPI (idéntica al transmisor)
uint8_t spi_mode = SPI_MODE_0;
uint8_t spi_bits = 8;
uint32_t spi_speed = 10000000;  // 10 MHz
...
void gpio_set(int gpio, int value) {
    ...
    if (val_fd < 0) {
        perror("Error configurando GPIO");
        exit(1);
    }
    dprintf(val_fd, "%d", value);
    close(val_fd);
}


// Transferencia SPI (idéntica al transmisor)
uint8_t spi_transfer(uint8_t data) {
    uint8_t rx_buf;
...
// Leer registro (compatible con transmisor)
uint8_t lora_read_reg(uint8_t reg) {
...
// Escribir registro (compatible con transmisor)
void lora_write_reg(uint8_t reg, uint8_t value) {
...
// Inicialización LoRa (compatible con transmisor)
void lora_init(long frequency) {
    ...
    // Configurar frecuencia (433 MHz - misma que transmisor)
...
int main() {
    // Inicializar LoRa a 433 MHz (igual que transmisor)
    lora_init(433000000);
    ...
    while (1) {
        ...
        usleep(1000);  // Pequeña pausa para reducir carga de CPU
    }
